...
export const getFollowers = async (walletId: string, page="1", limit: string=DEFAULT_LIMIT_PAGINATION, searchText?: string, verified?: boolean) => {
...
export const getFollowed = async (walletId: string, page="1", limit: string=DEFAULT_LIMIT_PAGINATION, searchText?: string, verified?: boolean) => {
    const pagination = {page, limit}
    const filter: any = {}
    if (searchText) filter.searchText = searchText
    if (verified !== undefined) filter.verified = verified
